...
Токены Telegram ботов хранятся в базе данных в зашифрованном виде и управляются через веб-интерфейс. Это обеспечивает:

- **Безопасность**: токены не хранятся в файлах конфигурации
- **Гибкость**: можно добавлять/удалять ботов без перезапуска
...
#### Алгоритм работы с ботами:

1. **Получение бота** из базы данных по ID
2. **Создание экземпляра** TelegramBot для обработки
3. **Выполнение операции** (webhook, обработка сообщений)
4. **Освобождение ресурсов** - экземпляр уничтожается после использования

**Преимущества подхода:**
- **Нет глобального состояния** - каждый запрос независим
- **Автоматическая синхронизация** - всегда актуальные данные из БД
- **Масштабируемость** - можно запускать несколько экземпляров сервера
- **Простота отладки** - нет скрытого состояния
